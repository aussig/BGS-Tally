            ...
            if self.bgstally.state.enable_overlay_system and current_activity is not None:
                if self.report_system_address is not None:
                    ...
                    if report_system is not None:
                        self.bgstally.overlay.display_message("system_info", self.bgstally.formatter.generate_text(current_activity, DiscordActivity.BOTH, False, [report_system['System']], lang=self.bgstally.state.discord_lang), fit_to_text=True, text_includes_title=True)
                    ...
                    if len(pinned_systems) == 1:
                        self.bgstally.overlay.display_message("system_info", self.bgstally.formatter.generate_text(current_activity, DiscordActivity.BOTH, False, pinned_systems, lang=self.bgstally.state.discord_lang), fit_to_text=True, text_includes_title=True, ttl_override=TIME_WORKER_PERIOD_S + 2)
                    elif len(pinned_systems) > 1:
                        self.bgstally.overlay.display_message("system_info", _("Pinned Systems") + "\n" + self.bgstally.formatter.generate_text(current_activity, DiscordActivity.BOTH, False, pinned_systems, lang=self.bgstally.state.discord_lang), fit_to_text=True, text_includes_title=True, ttl_override=TIME_WORKER_PERIOD_S + 2) # Overlay pinned systems message
