...
class UI:
    ...
    def get_prefs_frame(self, parent_frame: tk.Frame):
        ...
        row = 0
        nb.OptionMenu(discofr2, self.language, self.language.get(), *self.languages.values(), command=self._language_modified).grid(row=row, column=0, padx=10, pady=1, sticky=tk.W)
        nb.Label(discofr2, text=_("Format for Discord Posts")).grid(row=row, column=1, padx=(50,10), sticky=tk.W) # LANG: Preferences label
        nb.OptionMenu(discofr2, self.formatter, self.formatter.get(), *sorted(self.formatters.values()), command=self._formatter_modified).grid(row=row, column=2, padx=10, pady=1, sticky=tk.W)
