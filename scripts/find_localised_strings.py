...
Heavily based on https://github.com/EDCD/EDMarketConnector/blob/main/scripts/find_localised_strings.py
with the following modifications:

1. Incorporate special handling for base_types.json file
2. Improve comment finding regex to cope with lines with a non-comment '#' character
3. Look for the legacy __() and _() functions as well as modern tr() and translations()
4. Support for `# LANG: Ignore` to skip specific strings
5. Add alpha sorting of the strings in the output template

Note that 2, 4, and 5 have been submited to the EDMC project as PR #2587.
...
def find_calls_in_stmt(statement: ast.AST) -> list[ast.Call]:
    ...
    if isinstance(statement, ast.Call) and get_func_name(statement.func) in (
        ...
        if (
            ...
            or ast.unparse(statement).find("_") != -1
            or ast.unparse(statement).find("__") != -1
        ):
...
def scan_python_file(path: pathlib.Path) -> list[ast.Call]:
...
def scan_base_types_file(path: pathlib.Path) -> list[ast.Call]:
    """Scan a base_types.json file for translatable strings."""
    out: list[ast.Call] = []
    with open(path, encoding="utf-8") as base_types_file:
        base_types = json.load(base_types_file)
        translatable_keys = ['Type', 'Type (Listed as/under)', 'Category', 'Location', 'Building Type', 'Prerequisites', 'Pad', 'Facility Economy', 'Economy Influence']

        for base_type in base_types.values():
            for key in translatable_keys:
                value = base_type.get(key)
                if value:
                    ...
                    setattr(call, "comment", f"Colonisation '{key}' [base_types.json]")
                    out.append(call)
    return out
