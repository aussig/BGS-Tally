...
class WindowAPI:
    ...
    def __init__(self, bgstally: 'BGSTally', api:API):
        ...
        self.image_logo_etn = PhotoImage(file = path.join(self.bgstally.plugin_dir, FOLDER_ASSETS, "logo_etn.png"))
    ...
    def show(self, parent_frame:tk.Frame = None):
        ...
        api_info = self.bgstally.config.api('etn')
        frame_etn:ttk.Frame = ttk.Frame(frame_connection_buttons)
        frame_etn.pack(side=tk.LEFT)
        tk.Button(frame_etn, image=self.image_logo_etn, height=28, bg="Black", command=partial(self._autofill, 'etn')).pack(side=tk.TOP, padx=4)
        HyperlinkLabel(frame_etn, text=_("Website ⤴"), font=FONT_SMALL, url=api_info.get('url_website', ""), underline=True).pack(side=tk.BOTTOM, padx=4) # LANG: Label on API settings window
    ...
    def _autofill(self, site:str):
        ...
        """
        api_info:dict|None = self.bgstally.config.api(site)
